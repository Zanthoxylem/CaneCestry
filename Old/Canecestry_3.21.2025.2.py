...
@app.callback(
    ...
    [
        ...
        State('kinship-method-slider', 'value')  # add the slider
    ]
)
def generate_family_tree(n_clicks, lineage_mode, selected_line_name, generation_depth, method_choice):
    ...
    sorted_relatives_df = sort_pedigree_df(relatives_df)
        # Instead of compute_amatrix_diploid_revised:
    if method_choice == 0:
        subset_amatrix = compute_amatrix_diploid(sorted_relatives_df)
    elif method_choice == 1:
        subset_amatrix = compute_amatrix_polyploid(sorted_relatives_df)
    ...
    kinship_values = {line: subset_amatrix.loc[selected_line_name, line] for line in subset_amatrix.index}
    ...
    parent_count = {line: 0 for line in subset_ancestors}
    ...
    num_poly_founders = len(poly_founders)
    num_half_defined = len(half_defined)
    num_fully_defined = len(fully_defined)
    ...
    Poly Crosses: {num_poly_founders}
    Half-Defined Genotypes: {num_half_defined}
    Fully Defined Genotypes: {num_fully_defined}
    """
...
def handle_upload_and_save_corrections(upload_contents, save_clicks,
    ...
    global df  # <--- Declare global df at the beginning
    ...
    if triggered_id == 'add-ped-upload':
        ...
        if not filename.lower().endswith('.txt'):
            return dash.no_update, dash.no_update, "Please upload a .txt file (tab-delimited)."

        try:
            new_df = pd.read_csv(BytesIO(decoded), sep='\t', encoding='latin-1')
        except Exception as e:
            return dash.no_update, dash.no_update, f"Error reading file: {e}"
        ...
        return (found_invalid_rows, found_valid_rows, msg)

    elif triggered_id == 'add-ped-save-corrections-btn':
        if not invalid_rows:
            return dash.no_update, dash.no_update, "No invalid rows to correct."
