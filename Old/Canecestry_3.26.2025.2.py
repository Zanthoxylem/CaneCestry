...
def generate_family_tree(n_clicks, lineage_mode, selected_line_name, generation_depth, method_choice):
    ...
    # Now, compute the visual subset based on slider (generation depth)
    subset_nodes = set()
    ...
    # (Optional) You may wish to also subset the relationships so that both parent and child are in the visual subset.
    subset_relationships = [(p, c, role) for p, c, role in relationships if p in subset_nodes and c in subset_nodes]

    # The rest of your node classification remains the same.
    founders = set()
    poly_founders = set()
    half_defined = set()
    fully_defined = set()
    ...
    for parent, child, _ in subset_relationships:
        if child in parent_count:  # avoid key errors
    ...
    # Draw nodes using the full kinship values from the full matrix
    for line in subset_nodes:
